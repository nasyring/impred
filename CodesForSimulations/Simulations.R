...
###  IT IS NECESSARY TO MANUALLY CHANGE THE PRIOR HYPERPARAMETERS ON LINES 457 - 458
#
...
results <- list( oracle = matrix(NA, K, 18) , stdt = matrix(NA, K, 18), im = matrix(NA, K, 18), boot = matrix(NA, K, 18), para.boot = matrix(NA, K, 18), bayes.default = matrix(NA, K, 18), bayes.cauchy = matrix(NA, K, 18),bayes.oracle = matrix(NA, K, 18), conf = matrix(NA, K, 6)  )
for(k in 1:K){
  then = proc.time()
  ...
  para.boot.theta <- bootMer(lmer.model, para.boot.theta.func, nsim = 500, type = 'parametric')
  para.boot.new <- bootMer(lmer.model, para.boot.new.func, nsim = 500, type = 'parametric')
  para.boot.exs <- bootMer(lmer.model, para.boot.exs.func, nsim = 500, type = 'parametric')
  ...
  results$para.boot[k,13] <- ifelse(para.boot.pdi.theta.80[1]<=theta.star & para.boot.pdi.theta.80[2]>=theta.star, 1, 0)
  ...
  results$bayes.default[k,13] <- ifelse(bayes.default.pdi.theta.80[1]<=theta.star & bayes.default.pdi.theta.80[2]>=theta.star, 1, 0)
  ...
  pr2 <-  brms::prior(cauchy(0, sqrt(0.1)), class = "sd") + 
    brms::prior(cauchy(0, sqrt(1)), class = "sigma")
  bayes.oracle.posterior <- brm(response ~ 1 + (1 | group), data = my.data, prior = pr2)
  ...
  results$bayes.oracle[k,13] <- ifelse(bayes.oracle.pdi.theta.80[1]<=theta.star & bayes.oracle.pdi.theta.80[2]>=theta.star, 1, 0)
